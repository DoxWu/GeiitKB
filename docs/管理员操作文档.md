...
- **降级**：LLM 主模型不可用时自动切换备用模型，记录 `is_degraded=True`
...
**当前配置**：`celery_app.py` 第 67 行 `timezone="Asia/Shanghai"`，定时任务按北京时间 03:00 执行。
...
| 任务名 | 调度时间 | 功能 | 配置位置 |
|--------|----------|------|----------|
| `cleanup-expired-data` | 每日 03:00（Asia/Shanghai） | 清理过期对话/QA事件/邮件日志/审计日志 | [celery_app.py](file:///c:/Users/DOXIA/Desktop/企业知识库问答系统/kb_qa_system/backend/app/core/celery_app.py#L151) L151-L156 |
