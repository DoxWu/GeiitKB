...
```bash
# 1. 检查慢查询日志（postgresql.conf 已开启 log_min_duration_statement=1000）
docker-compose logs postgres | grep "duration:"

# 2. 检查 PG 配置是否生效
docker-compose exec postgres psql -U postgres -d kb_qa -c "
  SHOW shared_buffers;
  SHOW effective_cache_size;
  SHOW random_page_cost;
  SHOW work_mem;
"
# 预期：shared_buffers=128MB, effective_cache_size=384MB, random_page_cost=1.1, work_mem=8MB
...
| 原因 | 检查方法 | 解决方案 |
|------|----------|----------|
| `random_page_cost` 未调优 | `SHOW random_page_cost` 应为 1.1 | 检查 `postgresql.conf` 是否挂载 |
| 连接池耗尽 | `pg_stat_activity` 中 active 连接接近 `max_connections` | 调大 `DB_POOL_SIZE` 或 `max_connections` |
| IVFFlat 索引未使用 | `idx_scan` 为 0 | `SET ivfflat.probes = 10;` 或重建索引 |
| 慢查询 | 日志中出现 `duration: XXXX ms` | 优化查询语句或添加索引 |

#### Q2：应用启动慢（Railway 部署超时）

**排查步骤**：

```bash
# 1. 检查 MODEL_EAGER_HEALTH_CHECK 是否为 False
# Railway Dashboard → 后端服务 → Variables → MODEL_EAGER_HEALTH_CHECK

# 2. 检查启动日志中健康检查模式
# 应看到 "✅ 模型服务健康检查已调度（lazy 模式，后台启动中）"
# 而非 "已启动（eager 模式）"

# 3. 如果仍慢，检查 OPENAI_API_BASE 网络延迟
curl -w "%{time_connect}s" -o /dev/null -s $OPENAI_API_BASE
```

**解决**：确保 `MODEL_EAGER_HEALTH_CHECK=False`（默认值）。如仍慢，检查 LLM API 网络延迟或使用降级 API。
...
```bash
# 1. 检查 preconnect 标签是否生效
# 浏览器 DevTools → Network → 查看是否有 preconnect 请求

# 2. 检查 VITE_API_BASE_URL 是否正确
# 构建产物 index.html 中应包含 <link rel="preconnect" href="https://your-api...">

# 3. 检查 chunk 是否按需加载
# DevTools → Network → JS 资源应分多个 chunk 加载（非单个大文件）

# 4. 运行 Lighthouse 审计
npx lighthouse https://your-frontend.up.railway.app --view
```
...
| 原因 | 检查方法 | 解决方案 |
|------|----------|----------|
| preconnect 未生效 | DevTools 查看连接时序 | 确认 `VITE_API_BASE_URL` 已正确配置 |
| chunk 过大 | DevTools → Network → JS | 确认路由懒加载已启用（`React.lazy`） |
| 静态资源未缓存 | Response Headers 中无 `Cache-Control: immutable` | 检查 nginx 配置 `/assets/` 路径 |
| Service Worker 未注册 | DevTools → Application → Service Workers | 确认生产环境构建（`import.meta.env.PROD`） |
